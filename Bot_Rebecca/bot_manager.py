...
class LogWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        self.title("📋 Logs do Rebecca Bot")
...
class BotManager(ctk.CTk):
    def __init__(self):
        ...
        self.title("🤖 Rebecca Bot Manager")
        ...
        # Limpa processos órfãos do bot antes de iniciar
    ...
    def cleanup_orphan_processes(self):
        """Limpa processos órfãos do bot que ficaram rodando após desligamento inesperado"""
        try:
            import psutil

            # Nome do processo do bot (rebecca_bot.py rodando via python)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            main_path = os.path.join(script_dir, "rebecca_bot.py")

            killed_count = 0

            # Procura por processos Python rodando o rebecca_bot.py
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    cmdline = proc.info.get('cmdline')
                    if cmdline and len(cmdline) >= 2:
                        # Verifica se é um processo Python rodando nosso rebecca_bot.py
                        if 'python' in cmdline[0].lower() and main_path.lower() in ' '.join(cmdline).lower():
                            # Mata o processo órfão
                            proc.kill()
                            killed_count += 1
            ...
            if killed_count > 0:
                self.log(f"Total de {killed_count} processo(s) órfão(s) eliminado(s)")
            else:
                self.log("Nenhum processo órfão encontrado")

        except ImportError:
            # Se psutil não estiver instalado, faz limpeza básica apenas do arquivo PID
            self.log("AVISO: psutil não encontrado, fazendo limpeza básica")
            if os.path.exists(self.pid_file):
                os.remove(self.pid_file)
                self.log("Arquivo PID limpo")
        except Exception as e:
            self.log(f"Erro ao limpar processos órfãos: {str(e)}")

    def create_tray_icon(self):
        """Cria o ícone na bandeja do sistema"""

        # Desenha um ícone simples circular
    ...
    def setup_ui(self):
        ...
        # Título
        title = ctk.CTkLabel(self, text="🤖 Rebecca Bot Manager",
        ...
        self.stop_btn = ctk.CTkButton(btn_frame, text="⏹️ Parar",
        ...
        config_title = ctk.CTkLabel(config_frame, text="⚙️ Configurações",
    ...
    def export_log(self):
        ...
        try:
            ...
            script_dir = os.path.dirname(os.path.abspath(__file__))
            log_file = os.path.join(script_dir, "logs", "rebecca_bot.log")
            ...
            suggested_name = f"rebecca_bot_log_{timestamp}.log"
            ...
            dest_path = filedialog.asksaveasfilename(
                ...
                title="Exportar Log do Rebecca Bot"
            )
        ...
        if os.path.exists(self.pid_file):
            try:
                ...
                try:
                    ...
                    self.update_status("Ligado (2º Plano)", "#27ae60")
                    ...
                    os.remove(self.pid_file)
                    self.log("⚠️ Arquivo PID inválido removido")
            except:
                pass
    ...
    def start_bot(self):
        ...
        try:
            self.log("🚀 Iniciando Rebecca Bot...")
    ...
    def read_bot_logs(self):
        """Lê os logs do bot em tempo real"""
        import queue
        import time

        def enqueue_output(pipe, queue_obj, prefix):
            """Função auxiliar para ler pipe em thread separada"""
            try:
        ...
        try:
            ...
            # Threads para ler stdout e stderr de forma não-bloqueante
    ...
    def stop_bot(self):
        """Para o bot"""
        if not self.is_running:
            self.log("⚠️ Bot não está rodando!")
            return
        ...
        try:
            ...
            if os.path.exists(self.pid_file):
                ...
                try:
                    ...
                    self.log("⚠️ Processo já foi encerrado")
                    os.remove(self.pid_file)
    ...
    def schedule_auto_restart(self):
        """Agenda restart automático"""
        if self.auto_restart_var.get() and self.is_running:
            interval_ms = self.interval_var.get() * 60 * 60 * 1000
            self.log(f"⏰ Próximo restart em {self.interval_var.get()}h")
            self.after(interval_ms, self.auto_restart)

    def auto_restart(self):
        """Executa restart automático"""
        if self.auto_restart_var.get() and self.is_running:
            self.log("⏰ Restart automático programado")
            self.restart_bot()
    ...
    def on_closing(self):
        ...
        if self.is_running:
            ...
            self.log("ℹ️ Gerenciador ocultado na bandeja.")
        else:
            self.tray_icon.stop()
            self.destroy()
